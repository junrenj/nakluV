...
layout(set = 0, binding = 1) uniform sampler2D GBuffer1Tex;     // albedo.rgb, metalness.a
layout(set = 0, binding = 2) uniform sampler2D GBuffer2Tex;     // world position.xyz
layout(set = 0, binding = 3) uniform sampler2D DirectLightTex;  // linear direct lighting

layout(set = 0, binding = 4, std140) uniform SSDOParams
{
    mat4 ViewFromWorld;
    mat4 Projection;
    mat4 InvProjection;
...
void main()
{
    ...
    vec2 noiseInput = uv * vec2(textureSize(GBuffer0Tex, 0));
    vec3 randomVec = RandomVec(noiseInput);

    vec3 T = randomVec - N * dot(randomVec, N);
    if (length(T) < 1e-4)
    {
    ...
    for (int i = 0; i < UBO.SampleCount; ++i)
    {
        ...
        float scale = float(i + 1) / float(UBO.SampleCount);
        ...
        vec3 sampleDirect = texture(DirectLightTex, sampleUV).rgb;
        ...
        float receiverCos = max(dot(N, wi), 0.0);
        float senderCos = max(dot(sampleN, -wi), 0.0);

        if (receiverCos <= 0.0 || senderCos <= 0.0)
            continue;

        float dz = samplePVS.z - targetPosVS.z;

        float thickness = max(UBO.Bias * 3.0, 0.04);

        bool occluded = dz > UBO.Bias && dz < thickness;
        if (!occluded)
            continue;

        float rangeWeight = 1.0 - smoothstep(0.0, UBO.Radius, dist);
        float depthWeight = 1.0 - smoothstep(UBO.Bias, thickness, dz);

        float directScale = 5.0;
        float directLuma = dot(sampleDirect, vec3(0.2126, 0.7152, 0.0722)) * directScale;
        directLuma = clamp(directLuma, 0.0, 2.0);
        float geo = sqrt(max(receiverCos * senderCos, 0.0));
        float distWeight = 1.0 - smoothstep(0.0, UBO.Radius, dist);

        vec3 bounce = sampleAlb * directLuma * geo * distWeight * rangeWeight * depthWeight;

        indirect += bounce;
    }

    indirect *= (2 * PI / float(UBO.SampleCount));
    indirect *= UBO.Power;
