...
void UAnimPlayer::UpdateAnimation(UAnimInstance* AnimInstance, const float CurrentTime)
{
    ...
    switch (AnimInstance->Channel)
    {
        case UAnimInstance::EChannel::Rotation:
            ...
            AnimInstance->Node->Transform.Rotation = Result;
            break;
        case UAnimInstance::EChannel::Translation:
            ...
            AnimInstance->Node->Transform.Translation = glm::mix(V1, V2, Alpha);
            break;
        case UAnimInstance::EChannel::Scale:
            vec3 v1 = glm::make_vec3(AnimInstance->Values.data() + (3 * LeftIdx));
            vec3 v2 = glm::make_vec3(AnimInstance->Values.data() + (3 * RightIdx));
            AnimInstance->Node->Transform.Scale = glm::mix(v1, v2, Alpha);
            break;
    }
}
